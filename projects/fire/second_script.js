...
Map.addLayer(clipper.select(["Coarse_Resolution_Brightness_Temperature_Band_21"]),{},"collection",false)
...
var scaled = terra.map(scaling);

var terra_list = scaled.toList(scaled.size());

Map.addLayer(ee.Image(terra_list.get(number)).clip(AOI).select(["LST_Night_1km"]));
